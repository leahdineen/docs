...
```bash cURL
curl -X POST https://api.getpique.ai/judges \
...
```bash cURL
curl -X POST https://api.getpique.ai/content/upload \
  -H "X-API-Key: sk_org_xxxxxxxxx" \
...
response = requests.post(
    "https://api.getpique.ai/content/upload",
    headers={"X-API-Key": "sk_org_xxxxxxxxx"},
    files=files
)
...
response = requests.post(
    "https://api.getpique.ai/evaluations/single",
    headers=headers,
    json=data
)
...
response = requests.get(
    f"https://api.getpique.ai/evaluations/{evaluation['evaluation_id']}",
    headers={"X-API-Key": "sk_org_xxxxxxxxx"}
)
