...
```bash Request
curl -X GET https://api.getpique.ai/feedback/evaluation/eval_abc123 \
...
curl -X GET "https://api.getpique.ai/feedback/evaluation/eval_abc123?feedback_type=correction" \
  -H "X-API-Key: sk_org_xxxxxxxxx"
```
...
curl -X GET https://api.getpique.ai/feedback/evaluation/eval_invalid \
  -H "X-API-Key: sk_org_xxxxxxxxx"
```
